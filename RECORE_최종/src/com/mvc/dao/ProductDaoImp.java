...
//				System.out.println("imp의 order_num list : " + order_num);
	...
	// 상품상세페이지에서 리뷰글 출력하기위한 메소드
	@Override
	public List<Vo_Review> Prod_ReviewSelectAll(int pseq) {

		Connection con = getConnection();
		PreparedStatement pstm = null;
		ResultSet rs = null;
		List<Vo_Review> relist = new ArrayList<Vo_Review>();

		try {
			Properties prop = new Properties();
			String filePath = properties("query_product.properties");
			prop.load(new FileInputStream(filePath));
			String sql = prop.getProperty("selectProdReview");
			...
			while (rs.next()) {
				...
				Vo_Review tmp = new Vo_Review();
				tmp.setOrder_no(rs.getInt(1));
				tmp.setProd_id(rs.getInt(2));
				tmp.setReview_title(rs.getString(3));
				tmp.setReview_content(rs.getString(4));
				tmp.setReview_rate(rs.getInt(5));
				tmp.setReview_regdate(rs.getDate(6));
				rs.getString(7);
				tmp.setAcc_id(rs.getString(8));
	...
	@Override
	public boolean Point_update(int acc_no, int order_seq) {

		Connection con = getConnection();
		PreparedStatement pstm = null;
		int res = 0;

		String sql = "UPDATE ACCOUNT SET ACC_POINT = " + "((SELECT ACC_POINT FROM ACCOUNT WHERE ACC_NO = ?) - "
				+ "(SELECT ORDER_POINT FROM ORDER_NUM WHERE ORDER_NO = ?))";

		try {
			pstm = con.prepareStatement(sql);
			pstm.setInt(1, acc_no);
			pstm.setInt(2, order_seq);
			res = pstm.executeUpdate();
