			...
			if (id.equals(vo.getAcc_id()) && pw.equals(vo.getAcc_pw())) {
				...
				if (vo.getAcc_isReg().equals("Y")) {
					...
					if (vo.getAcc_m_c().equals("M")) { // 관리자페이지
						...
						dispatch("issue.do?command=main", request, response);
					}
				} else {
...
/*			switch(acc_birth_day) {
			...
			if (res) {
				/* dispatch("./RECOREMain/index.html", request, response); */
			...
			if(res) {
				jsResponse("회원정보 수정 성공", "./mypage.do?command=main", response);	
			}
			...
			String res = dao.A_selectAccountPwChk(id, name, email);
			...
			String res = dao.A_selectAccountIdChkMail(name, email);
		...
		} else if(command.equals("idphone")) {
			String name = request.getParameter("name");
			String phone = request.getParameter("mobile");
		
			String res = dao.A_selectAccountIdChkPhone(name, phone);
