...
//      session.setAttribute("vo", vo);
      ...
      if(command.equals("main")){ //main
         ...
         int catd_no = Integer.parseInt(request.getParameter("catdno"));
         
         //조회수 업데이트
         boolean res = mdao.My_updateBoard_Viewcount(qna_no);
         
         if(catd_no == 16) { //상품 문의일 때
        	 //qna 객체 가져오기
        	 Vo_QnA vo_qna = mdao.My_selectBoardProdOne(qna_no);
        	 request.setAttribute("vo_qna", vo_qna);

        	 //상품 정보 뿌리기용
        	 Vo_Product vo_prod = mdao.selectProd(vo_qna.getQna_seq_no());
